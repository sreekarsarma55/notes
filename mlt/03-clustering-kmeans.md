...
Split `n` points into `K` clusters, each with a centroid `mu_k`. Minimise the **within-cluster sum of squares** (distortion):
...
3. Pick the next centroid with probability PROPORTIONAL to D(x)^2.
4. Repeat 2-3 until K centroids, then run normal Lloyd's.
```

The probability a specific point is chosen next:
```
P(x chosen) = D(x)^2 / sum_over_all_points D(.)^2
```
...
Points `{1, 2, 9, 10}`, K = 2, initial `mu1 = 1`, `mu2 = 2`.

**Assign:** point 1 → C1; points 2, 9, 10 → nearer to 2 → C2.
```
C1 = {1},  C2 = {2, 9, 10}
```
**Update:** `mu1 = 1`, `mu2 = (2+9+10)/3 = 7`.

Next assign with `mu1=1, mu2=7`: points 1,2 → C1; 9,10 → C2 → `C1={1,2}, C2={9,10}` → `mu1=1.5, mu2=9.5`. One more pass leaves it unchanged → **converged**: clusters `{1,2}` and `{9,10}`.

## 9. Worked example — computing J

Clusters `{2,3,4}` with `mu=3` and `{10,12,20,30,11,25}` with `mu=18`:
```
Cluster 1: (2-3)^2 + (3-3)^2 + (4-3)^2 = 2
Cluster 2: 8^2 + 6^2 + 2^2 + 12^2 + 7^2 + 7^2 = 346
J = 2 + 346 = 348
```
...
| What does K-means++ change? | **Initialisation only** (prob ∝ D(x)^2). |
